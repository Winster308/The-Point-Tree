...
- display-text：显示一些文本（可以使用基本 HTML）。参数是要显示的文本。也可以是一个返回动态更新文本的函数。
...
- blank：添加空白空间。默认尺寸为 8px x 17px。参数可改变尺寸。如果是一个值（例如 "20px"），则决定高度。如果有两个参数，第一个是宽度，第二个是高度。

- row：水平显示一组组件。参数是一个采用标签页布局格式的组件数组。

- column：垂直显示一组组件。参数是一个采用标签页布局格式的组件数组。这在行内显示列时非常有用。
...
- prestige-button：用于重置该图层货币的按钮。

- text-input：一个文本输入框。参数是输入对应的 player[layer] 中的变量名，即 player[layer][argument]。
    （支持字符串、数字和 Decimal！）

- slider：允许用户通过滑块输入值。参数是一个 3 元素数组：[名称, 最小值, 最大值]。
    名称是输入对应的 player[layer] 中的变量名，最小值和最大值是滑块的限制范围。
    （不适用于 Decimal 值）

- drop-down：允许用户通过下拉菜单输入值。参数是一个 2 元素数组：[名称, 选项]。
    名称是输入对应的 player[layer] 中的变量名，选项是一个字符串数组，表示可用的选项。

- upgrades、milestones、challenges、achievements、buyables、clickables：相应地显示图层的升级、挑战等。参数是可选的，如果组件不包含所有行，则参数是此组件应包含的行列表。
...
- toggle：一个切换按钮，用于切换布尔值。参数是一对标识，用于定位 player 中要切换的布尔值的位置，例如 `[layer, id]`。'layer' 也会影响切换按钮的颜色。

- grid：显示该图层的可网格化网格。如果你需要多个网格，请使用图层代理。参数是可选的，如果组件不包含所有行，则参数是此组件应包含的行列表。

- layer-proxy：允许你使用其他图层的组件。参数是一对 `[layer, data]`，包含要代理的图层的 id，以及要显示的组件的 tabFormat。
    （注意：你不能在图层代理中使用子标签页）
