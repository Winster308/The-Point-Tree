...
别让“Github”这个词吓到你。实际上，它比大多数人想象的要容易得多，尤其是因为大多数人用了复杂的方式。关键在于Github Desktop，它能让你完成所有必要操作，甚至无需触碰命令行。

使用Github的好处：

- 它让更新模组树变得极其简单。
- 你可以通过githack轻松分享你的作品，或者稍加努力，搭建一个github.io网站。
- 它可以让你撤销代码更改，并保留多个版本。
- 如果你想，还能与他人协作。
...
3. 在浏览器中登录，然后返回[模组树页面](https://github.com/Acamaeda/The-Modding-Tree)。右上角应有一个写着“fork”的按钮。点击它，然后选择你的用户名。现在，你拥有了自己的模组树分支，即副本。

4. 打开Github Desktop并登录。忽略其他所有内容，选择“clone a repository”（克隆仓库）。“仓库”基本上就是“Github项目”，比如模组树。“克隆”就是将仓库的副本下载到你的电脑上。

5. 在仓库列表中找到模组树（它应该是唯一的一个），然后点击“clone”（克隆）。

6. 选择你是为了自己的目的使用它，然后点击“continue”（继续）。它将下载文件并处理一切。

### 使用你的仓库

1. 点击右侧的“show in explorer/finder”（在资源管理器中显示），然后打开文件夹中的index.html文件。页面应在你的浏览器中打开。这将让你在本地查看和测试你的项目！

2. 要编辑你的项目，在Github Desktop中点击“open in VSCode”（在VSCode中打开）。
...
4. 保存[mod.js](/js/mod.js)，然后在浏览器中重新加载[index.html](/index.html)。标签页标题以及信息页面上的标题现在都会更新！**每次更改代码后，你都可以重新加载页面以快速测试。**
...
现在，你已经成功使用了Github！你可以查看关于[制作模组](making-a-mod.md)的下一个教程，或查看[文档](/documentation/!general-info.md)以了解模组树的系统运作方式，并实现你的模组构想。