...
function rowReset(row, layer) {
	for (lr in ROW_LAYERS[row]){
		if(layers[lr].doReset) {
			if (!isNaN(row)) Vue.set(player[lr], "activeChallenge", null) // 在相同或更高行重置时退出挑战
...
function resetRow(row) {
	if (prompt('您确定要重置此行吗？强烈建议您等到当前运行结束后再执行此操作！请输入"我要重置此行"以确认')!="我要重置此行") return
